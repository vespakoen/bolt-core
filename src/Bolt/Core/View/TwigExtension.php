...
class TwigExtension extends \Twig_Extension
{
    ...
    public function listcontent($contenttype, $relationoptions, $content)
    {
        ...
        if (!empty($relationoptions['order'])) {
            ...
            $options['self_paginated'] = false;
        }

        $results = array();
        $contents = $this->app['db']->fetchAll("select * FROM $contenttype"); //getContent($contenttype, $options);
        foreach($contents as $content) {
            $results[] = new Content($this->app, $contenttype, $content);
        }
    ...
    public function htmlencode_params($params)
    {
        $result = array();
        foreach ($params as $key => $val) {
            $result[$key] = $this->htmlencode($val);
        }
        return $result;
    }
    ...
    public function trans()
    {
        ...
        switch ($num_args) {
            case 5:
                return $this->__($args[0], $args[1], $args[2], $args[3], $args[4]);
            case 4:
                return $this->__($args[0], $args[1], $args[2], $args[3]);
            case 3:
                return $this->__($args[0], $args[1], $args[2]);
            case 2:
                return $this->__($args[0], $args[1]);
            case 1:
                return $this->__($args[0]);
        }
     ...
     * @return string
     */
    public function path($path, $param = array(), $add = '')
    {
        if (!empty($add) && $add[0] != "?") {
            $add = "?" . $add;
        }
        ...
        return $this->app['url_generator']->generate($path, $param). $add;
    }
